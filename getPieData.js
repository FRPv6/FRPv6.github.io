...
let json_file_list=["as", "category", "sub_category", "org_name"]
...
for(let i=0;i<4;i++){
    ...
    for(let j=0;j<3;j++){
            $.getJSON(file_name, function (chart) {
                let index=j
                let element_id=`pie-${class_list[j]}-${id_type_list[i]}`
                let lineChart = echarts.init(document.getElementById(element_id))
                chart_list[i].push(lineChart)
                let select_id=document.getElementById(`select-${class_list[j]}-${id_type_list[i]}`)
                let select_value=select_id.value
                console.log(i,j,`select-${class_list[j]}-${id_type_list[i]}`,select_value)
                        ...
                        feature:{
                            dataView:{
                                ...
                                optionToContent: function(opt) {
                                    let table_id=`table-${class_list[j]}-${id_type_list[i]}`
...
for(let i=0;i<4;i++){
    ...
    for(let j=0;j<3;j++){
            ...
            select_id.addEventListener('change', event => {
                let element_id=document.getElementById(`pie-${class_list[j]}-${id_type_list[i]}`)
                // 获取当前选中的选项的值
                ...
                $.getJSON(file_name, function (chart) {
                    ...
                    console.log(`pie-${class_list[j]}-${id_type_list[i]}`)
